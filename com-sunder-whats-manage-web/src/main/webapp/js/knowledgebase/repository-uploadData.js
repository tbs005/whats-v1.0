$(function() {
	...
	WebUploader.Uploader.register({
		...
		beforeSend : function(block) {
			...
			$.ajax({
				type : "POST",
				url : contentpath + "/knowledgeFile/check",
				data : {
					// 文件唯一标记
					fileMd5 : fileMd5,
					// 当前分块下标
			...
			this.owner.options.formData.typeId = parentId;
			this.owner.options.formData.name = $('#attachName').val();
		...
		afterSendFile : function() {
			...
			$.ajax({
				type : "POST",
				url : contentpath + "/knowledgeFile/mergeChunks",
				data : {
					fileMd5 : fileMd5,
					filename : filename,
					chunk : chunk,
				...
				success : function(data) {
					...
					if (data.errcode == 0) {
	    				$.prompt("上传成功",function(){
	    					location.href=contentpath+"/knowledgeFile/adminList?parentId="+parentId;
	    				});
					} else {
						$.prompt("系统错误!");
					}
	...
	var uploader = WebUploader.create({
		...
		duplicate : true,
		// 文件接收服务端。
		server : contentpath + '/knowledgeFile/materialUpload',
		// 选择文件的按钮。可选。
